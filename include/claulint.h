...
class Lint { // namespace -> class??, some part -> main?, call run()?, make run method?
public:
	Lint() : builder(102400), builder2(102400)
	{
...
private:
	...
	wiz::load_data::UserType* clautextUT = nullptr;
	wiz::StringBuilder builder;
	wiz::StringBuilder builder2;
	...
	public:
		...
		enum class Id_ { _NONE, ID, TOTAL_ID };
		enum class OneMore_ { _NONE, ONEMORE, JUSTONE };
		enum class Required_ { REQUIRED, OPTIONAL_ };
		enum class EmptyUT_ { _NONE, OFF, ON };

		enum class Order_ { _NONE, OFF, ON }; // with global??
		enum class Multiple_ { _NONE, OFF, ON };
	public:
		...
		std::string prefix;
	public:
		Option() : type(), //id(Id_::_NONE), 
			//onemore(OneMore_::_NONE), 
			required(Required_::REQUIRED),
			empty_ut(EmptyUT_::_NONE)
		{
			//
		}
	public:
		...
		const wiz::load_data::ItemType<WIZ_STRING_TYPE>& x, const wiz::load_data::ItemType<WIZ_STRING_TYPE>& y, const std::string& real_dir) //, Order?
	{
		...
		if (name_do && val_do) {
			...
			wiz::StringTokenizer tokenizer2(val_option.event_ids, &builder2);
			...
			while (tokenizer.hasMoreTokens()) {
				...
				wiz::ExcuteData excuteData;
				wiz::ArrayMap<std::string, std::string> parameters;
				parameters.insert(std::pair("name", y.GetName().ToString()));
				parameters.insert(std::pair("value", y.Get(0).ToString()));
				parameters.insert(std::pair("is_usertype", "FALSE"));
				parameters.insert(std::pair("real_dir", real_dir));
				parameters.insert(std::pair("select", "NAME"));
				// check input!
				parameters.insert(std::pair("input", y.GetName().ToString()));
				std::pair<bool, wiz::DataType> result = functionMap[event_name](schema_eventUT, excuteData, parameters);
				if (result.first && "TRUE" == result.second.ToString()) {
					//
				}
				else {
					//
			...
			while (tokenizer2.hasMoreTokens()) {
				...
				parameters.insert(std::pair("is_usertype", "FALSE"));
				parameters.insert(std::pair("real_dir", real_dir));
				parameters.insert(std::pair("select", "VALUE"));
				// check input!
				parameters.insert(std::pair("input", y.Get(0).ToString()));
	...
	)
	{
		...
		if (name_do) {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				wiz::ArrayMap<std::string, std::string> parameters;
				parameters.insert(std::pair("name", y->GetName().ToString()));
				parameters.insert(std::pair("is_usertype", "TRUE"));
				parameters.insert(std::pair("real_dir", real_dir));
				parameters.insert(std::pair("select", "NAME"));
				parameters.insert(std::pair("input", y->GetName().ToString()));
...
public:
	bool Init(const wiz::load_data::UserType& schemaFileUT)
	{
		ClauToCpp::Init();
		functionMap = ClauToCpp::make;
